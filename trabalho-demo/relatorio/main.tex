...
\begin{algorithm}[ht]
   ...
   \myproc{\proc{$s^*$, numVezes}}{
      ...
      \For {$i \gets 1$ até $numVezes$} {
         selecione tarefas $j_1 \neq j_2$, aleatoriamente \;
         $\bar{s} \gets$ troque a ordem de processamento de $j_1 \leftrightarrow j_2$ \;
   ...
   \caption{Algoritmo de Busca Local iterada com trocas aleatória de duas tarefas.}
\end{algorithm}
...
\begin{algorithm}[ht]
   ...
   \myproc{\proc{$N$, $M$, $T$, $\alpha$}}{
      $\mathit{pend} \gets \{1,\dots, N$\} \;
      $s \gets $ lista vazia; $z \gets 0$ \;
      \While{$\mathit{pend}$ não está vazio}{
         ...
         $\mathit{tp} \gets$ escolhe tupla aleatória dos 
            índices $[1, \max\{1, \alpha \cdot \mathit{tam}\} ]$ \;
         adicione a tarefa e atualize o custo $z$ com $\mathit{RCL}_\mathit{tp}$ \;
      }
   }
   \Return{$s$}
   \caption{Algoritmo de Busca Local iterada com trocas aleatória de duas tarefas.}
